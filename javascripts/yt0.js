...
//  var fcnm = rpt.match(/signature=([^(]+)/)[1]
  ...
  function fcobj(){
    var mch = rpt.match(fs)[0].split('\,');alert(mch + typeof mch); var mch = mch.split('');for (j=0;j<mch.length;j++){if (mch[j] === "$") {alert(mch[j]);mch[j]="\\$";alert(mch[j]) }}; var mch = mch.join(''); alert(mch)
