    ...
    var code = "jQuery(document).ready(function($) {\
		...
		pauseOtherPlayers: false,\
                ...
                success: function(me) {  $('#audio-type').html( me.pluginType);\
					...
					me.addEventListener('pause', function() { document.getElementsByClassName('play')[0].innerHTML = 'play'; if (me.src.replace('&ratebypass=yes','') != player2.src.replace('&ratebypass=yes','')) { if (Seek == 3) { player2.pause() }; if (Seek === 0) { Seek = 1 }; player2.currentTime = me.currentTime; player2.pause();if (Srcto == Audio) { alert();me.setCurrentTime( me.currentTime ) } }});\
