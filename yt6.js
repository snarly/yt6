  ...
  if (hide || (p && (p.parentNode.getAttribute('id') == 'player-api' || p.parentNode.getAttribute('id') == 'ytd-player' || p.parentNode.getAttribute('id') == 'player-container')) ) {
	...
	if (!hide) {
	  if (p && p.getAttribute('time')) {
		...
		y.appendChild(p)
		p.setAttribute('id','movie_player2')
	    }
	} else	{
		if (yt6.layout != 16 || yt6.ytg) {
...
function getPoster(){
  ...
  if (yt6.layout == 12) {
	...
	for(i=0;i<tn.length;i++){ if (typeof ytplayer.config.args[tn[i]] == 'string' && ytplayer.config.args[tn[i]].indexOf(yt6.vid) != -1) poster2 = ytplayer.config.args[tn[i]]; if (poster2 && (!poster1 || (poster1 && poster1.indexOf(tn[i].replace('iurl','')) != -1)) ) { poster = poster2; poster1 = null } else poster = poster1; } console.log(i + ' ' + poster2 + ' '+ poster)*/
	    ...
	    poster = protocol() + '//i.ytimg.com/vi/' + window.ytplayer.config.args.video_id + '/maxresdefault.jpg'
...
function correct_flashvars(a) { // prepare the flash player object's flashvars variable
  ...
  for (i in ft) {
	...
	if (qs.s){
	  ...
	  if (z[j].indexOf('lsig%3D') != -1) {
	    var signame = 'sig'
	    var sig = encodeURIComponent(dc(qs.s)).split('%').join('@@@@')
	  } else {
	  ...
	  if (c == 1) {
	    z[j] = z[j].replace('videoplayback%3F','videoplayback%3F' + signame + '%3D' + sig +'%26').split('s=' + qs.s).join('s=')
  ...
  url_encoded_fmt_stream_map = url_encoded_fmt_stream_map.split('%40%40%40%40').join('%')
...
//console.log(yt.player +' '+ ytplayer.config +' '+ ytplayer.config.args +' "'+ ytplayer.config.args.url_encoded_fmt_stream_map +'" '+ ytplayer.config.args.adaptive_fmts)
	      if (yt6.blocked) if (yt.player && ytplayer.config && ytplayer.config.args && (ytplayer.config.args.url_encoded_fmt_stream_map || ytplayer.config.args.adaptive_fmts)) { // && yt6.layout != 16)
			...
			//} //else if (z.name == 're-embed2') z.parentNode.removeChild(z)
			  ...
			  for(i=0;i<z.length;i++){
			    if(z[i] && z[i].getAttribute('id') == 'movie_player4_iframe') {
			      ...
			      if (player() && yt6.movie_player.firstChild && yt6.movie_player.firstChild.getAttribute('class')) {
				try { yt6.movie_player.pauseVideo(); yt6.movie_player.firstChild.firstChild.pause() } catch(e){}
				yt6.movie_player.firstChild.setAttribute('class','html5-video-container0')
				yt6.movie_player.firstChild.firstChild.setAttribute('class','video-stream-html5-main-video0')
...
//yt.player.Application.create(gid('player-container'), ytplayer.config);
			...
			if (iframe) {//if (z.re2 != true) {
			  if (yt6.movie_player.tagName == 'EMBED' && yt6.movie_player.getAttribute('src') == '' && yt6.movie_player.getAttribute('flashvars') == '' && yt6.movie_player.parentNode != y) {
			...
			function(){
			  if (player() == null) {
				...
				//yt6.change = yt6.blocked
				  ...
				  if (gid('bm0') && gid('bm0').parentNode.getAttribute('id') == 'movie_player_to_insert') {
				    yt6.api.appendChild(gid('bm0'))
				    if (yt6.status == 'fail') { alt_yt() } else if (yt6.x) { switch_players(); switch_players(); if (autoplay(true)) yt6.player1.play() }
				  }
				  //yt6.mep_reload()
...
function back2html5() {
			...
			if (gid('movie_player_to_dispose')) {
			  ...
			  for(var i in y) {
			    var x = y[i]; var xname = (typeof x.getAttribute == 'function') ? x.getAttribute('name') : x.name
...
//console.log('?'+offset)
  if (offset) {
    ...
    if (yt6.layout == 16 && !yt6.ytg && playlist_scroller.nextSibling.tagName == 'IRON-LIST' && playlist_scroller.nextSibling.id == 'items') {
...
if (typeof html.splice != 'function') return void 0;
  ...
  var onclic = "var yt6 = gid('snarls_player'); if (this.checked) { yt6.force_flash = true; forceFlashplayerObject(); switch_players(); switch_players(); $waitUntil(function(){var mp = getElementsByAttribute(document, 'div','name','original')[0]; if (mp && mp.parentNode.getAttribute('id') == 'movie_player_to_dispose') { return true } else console.log(mp) }, function() {var mp = getElementsByAttribute(document, 'div','name','original')[0]; mp.setAttribute('id','movie_player0'); mp.setAttribute('class','YouTube_TM'); },250,5000); } else { yt6.force_flash = false; back2html5() };" //mp.firstChild.setAttribute('class','html5-video-container0'); mp.firstChild.firstChild.setAttribute('class','video-stream html5-main-video0'); 
  dw.setAttribute('onclick', onclic)
  dw.setAttribute('style','inline-block')
  if (yt6.force_flash) {
    dw.setAttribute('checked','checked')
...
//}//cw
  ...
  if (yt6.force_flash) {
	...
	if (!(yt6.layout == 12 && yt6.blocked) && !yt6.html5_fail) { relocate_mep() } else gid('player-api').appendChild(bm0)
			...
			$waitUntil(function(){//var z = gid('movie_player0')
			  ...
			  if (p && p.nodeName != 'EMBED' && p.nodeName != 'IFRAME' && !gid('movie_player1')) { //console.log('#3')
			    ...
			    if (player()) player().setAttribute('id','movie_player0');
			      ...
			      if (!p2 && yt6.movie_player.id.indexOf('movie_player_uid_') == 0) var p2 = gid(yt6.movie_player.id)
  ...
  if (yt6.mpb.hasAttribute('active')) { // Opera error handling
    ...
    if (mpb && p.getAttribute('class').indexOf('ytp-player-minimized') > -1 && yt6.movie_player.parentNode == gid('player-container')) {
      ...
      mpb.insertBefore(p, mpb.firstChild)
		...
		if (yt6.flexy) if (!yt6.fullscreen && !yt6.ytp_fullscreen && (mep.style.width != '100%' || mep.style.height != '100%')) {
...
if (yt6.flexy && yt6.pls) {
  if (parseInt(window.innerWidth || document.documentElement.clientWidth || yt6.body.clientWidth) - yt6.sb >= 1000) {
	...
	if (yt6.ytm && (document.title == 'YouTube Music' || document.title == location.href) && (window.ytplayer && ytplayer.config && ytplayer.config.args && yt6.vid == video_id()[0]) ) {
...
//	if ((gid('placeholder-player') != null) && (bm0 != null) && (bm0.style.width != fix_Width()) ) aspect(yt6.size)
	...
	//if ((watch != null) && (strPrevLocation != yt6.strLocation)) {
	  ...
	  if (window.ytplayer && window.ytplayer.config && window.ytplayer.config.args && window.ytplayer.config.args.video_id) {
		...
		} else if (yt6.layout == 16 && yt6.newin && yt6.newin != true && yt6.pls && strPrevLocation.indexOf(yt6.newin) > -1) if (yt6.age.check()) { ajax1(true); try { yt6.newin = window.open(protocol() + '//www.youtube.com/watch?v=' + yt6.newin).blur() } catch(e) { return void 0 }; window.focus() } else delete yt6.newin;
...
function control_panel1() {
  ...
  if (gid('remove-sp') == null) {
    ...
    remove.id = 'remove-sp';
    var remove_sp_innerhtml = '<button onclick="switch_players()" class="snarl-button yt-uix-button-text" aria-label="Switch" title="Switch" style="width: 22px; height: 22px; padding: 0 1px 0 0"><img src="//s.ytimg.com/yts/img/HTML5_1Color_Black-vfl902gVJ.png" style="vertical-align:middle; height:12px; padding:0px""></img></button><button onclick="aspect()" class="snarl-button yt-uix-button-text" style="width: 30px" aria-label="Resize" title="Resize">«↔»</button><br><button style="width: 22px" onclick="var yt6 = gid(\'snarls_player\'); var z = window.prompt(\'Wanna quit? \(Exit options: 0-4\)\\n Cancel = Not now;\\n 0 = Remove script without page refresh;\\n 1 = Classic layout;\\n 2 = Material Design layout;\\n 3 = Visit page on YT Gaming;\\n 4 = Visit page on YT Music;\', \'0\'); if (!z) var z = window.confirm(\'OK will attempt to switch between YouTube layouts and refresh the page. \\nCancel will abort.\'); if (typeof z == \'boolean\' && z == false) return void 0; if (z && !(typeof z == \'boolean\' && z == true)) { if (z != 0 && (z == 1 || z == 2 || z == 3 || z == 4)) { if (z == 2) document.cookie=&quot;PREF=f1=50000000&f5=10&f6=1004;path=/;domain=.youtube.com;expires=Session&quot;; if (z == 1) document.cookie=&quot;PREF=f1=50000000&f5=10&f6=8;path=/;domain=.youtube.com;expires=Session&quot;; if (z == 3 || z == 4) { var domain = (z == 3) ? \'gaming\' : \'music\'; location.href = window.location.href.replace(\'www.youtube.com\', domain + \'.youtube.com\').replace(\'music.youtube.com\', domain + \'.youtube.com\').replace(\'gaming.youtube.com\', domain + \'.youtube.com\') } else location.href = window.location.href.replace(\'gaming.youtube.com\',\'www.youtube.com\').replace(\'music.youtube.com\',\'www.youtube.com\') } else if (z == 0) deldiv() } else { if (!yt6.ytg) if (yt6.layout == 16) { document.cookie=&quot;PREF=f1=50000000&f5=10&f6=8;path=/;domain=.youtube.com;expires=Session&quot;; } else document.cookie=&quot;PREF=f1=50000000&f5=10&f6=1004;path=/;domain=.youtube.com;expires=Session&quot;; location.href = window.location.href.replace(\'gaming.youtube.com\',\'www.youtube.com\').replace(\'music.youtube.com\',\'www.youtube.com\'); };" class="snarl-button yt-uix-button-text" aria-label="Remove script / Change YouTube layout" title="Remove script / Change YouTube layout">X</button><button id="audio_x" class="snarl-button yt-uix-button-text" aria-label="External audio" title="External audio" style="padding: 0 1px 0 1px; right: 0px" onclick="var yt6 = gid(\'snarls_player\'); if (yt6.player1 && yt6.player2 && typeof yt6.player2.load == \'function\') { var audio_x = window.prompt(\'Enter URL of any audio (or video) file across the net to use as background sound. Only plays alongside non-MSE-mode video-only sources.\', gid(\'no2\').src); yt6.audiox = gid(\'no2\').src = yt6.player2.media.src = audio_x || yt6.linx[yt6.userprefA[0]]; if (!audio_x) { yt6.audiox = null; delete yt6.audiox }; yt6.player1.pause(); yt6.player2.load() };">audio</button></div>'
