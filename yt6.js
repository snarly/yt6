							...
							//try { yt6.player1.media.currentTime = player2.currentTime = 0 } catch(e) { yt6.player1.media.currentTime = 0 };
						      ...
						      function(){ var y = yt6.pl_index, zi = yt6.pl_next; 
								...
								var mp = yt6.movie_player
						          ...
						          if (!yt6.ytg) {
							    if (!yt6.shuffle) {
							      try { yt6.navigation = true; mp.seekTo(mp.getDuration()+1) || zi.click() } catch(e) { if (mp.getAttribute('class') != 'forced flashplayer') { mp.playVideoAt(yt6.pl_index+1) || mp.nextVideo() } else try { zi.click() } catch(e){ yt6.navigation = false } }
							    } else {
...
//	if ((document.getElementById('placeholder-player') != null) && (bm0 != null) && (bm0.style.width != fix_Width()) ) aspect(yt6.size)
	...
	if (typeof yt6.navigation != 'number' && yt6.title && (yt6.title.split(' ').join('') != document.title.split(' ').join('') || yt6.strLocation != window.location.href) && document.title != 'YouTube') {//.split(' ').join('')
