      ...
      if (typeof poster != 'undefined') {
		...
		pauseOtherPlayers: false,
                ...
                success: function(me) {  $('#audio-type').html( me.pluginType);
					...
					me.addEventListener('ended', function() { if (player1.options.loop1) { Seek = 1; if (player2.currentTime == 0) { me.play() }; if (!isNaN(player2.duration)) { player2.currentTime = player2.duration } } else { Seek = 3; if ((player2.currentTime > 0) && (!isNaN(player2.duration))) { player2.currentTime = player2.duration };
                                          if (autoplay()) { ;;
