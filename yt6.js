...
function fix_playlist() {
  ...
  var p = player(), p = yt6.p
  var playlist = (!yt6.mobile) ? gid('watch-appbar-playlist') || gid('playlist') || gid('queue') || document.getElementsByTagName('ytg-playlist-panel-renderer')[0] : document.getElementsByTagName('lazy-list')[0]//gc('playlist-controls-primary')[0]
  if (yt6.layout == 16 && playlist && ((!playlist.lastChild || !playlist.firstElementChild) || playlist.hasAttribute('hidden')) ) playlist = null
...
*/
  ...
  for (i in ft) {
	...
	function alt_linx(index_) {
	  if (!(typeof linx[index_] == 'string' && linx[index_].indexOf('googlevideo.com/') > -1) ) {
	    return index_
	  } else return (1000 + 1 * index_)
	}
	    ...
	    linx[alt_linx(qs.itag)] = href

	  } else {

	      linx[alt_linx(qs.itag)] = linx_bak.split('&title=')[0]

	    }
        } else {
            if (!linx[241] || !(linx[241] && linx[241].indexOf('&title=Advertisement') > -1)) {

	      linx[alt_linx(241)] = href

	    } else {

		linx[alt_linx(241)] = linx_bak.split('&title=')[0]
...
/*					      var dash = gid(mep_x('mep_') + '_sourcechooser_242')//240p  VP9webm');
						...
						//if (!yt6.ytg || (yt6.ytg && yt6.x && (typeof yt6.errcount == 'undefined' || yt6.errcount == 0))) { decryption_failure() }
					  	  ...
					  	  // Attempting to replace unusable links with the alternative set
						  ...
						  for (l=0;l<b.children.length;l++){ 
						    if (typeof b.children[l].name == 'number' && b.children[l].href) {//itag_
						...
						(typeof srcto != 'undefined' && Srcto == Audio)
						  ...
						  if (!yt6.A_V[itag(me.src)] && player2.duration != player2.currentTime && !(me.currentTime > player2.duration)) {// && yt6.browser_tab == 'visible'
							...
							if (yt6.sync_timer > 5 && me.loaded) {
							  yt6.sync_timer = 0
							} else if (yt6.browser_tab != "hidden") { try {//if (yt6.V_[itag(me.src)]) {
							    //if (itag(me.src) != 278) {
								if (me.currentTime > 1) { Seek = 1; me.pause(); yt6.Seek = 2 } else Seek = 2//Seek = 1
							    //} else Seek = null;
								...
								//    me.play()
								    $waitUntil(
								      function(){
									if (yt6.Seek != 1 && yt6.player1.media.paused) return true
								      },
								      function() {
								        yt6.player1.play();//console.log('play')
								      },200,200)
								    //}
							          } catch(e) {} } else { me.currentTime = yt6.player1.media.currentTime = yt6.ct = player2.currentTime = yt6.player2.media.currentTime; }
						      },250,1250)
						    } else {
							if (yt6.browser_tab == "hidden" && Seek != 3) {
							  Seek = 5; yt6.ct = me.currentTime = yt6.player1.media.currentTime = yt6.player2.media.currentTime
							} else {
							    if (Seek == 1) { yt6.Seek = 1; yt6.player2.pause() }
...
//      if (Math.abs(p1.style.height.replace('px','') - Math.floor(p1.style.height.replace('px','') / 2) * 2) < Math.abs(p1.style.height.replace('px','') - Math.ceil(p1.style.height.replace('px','') / 2) * 2)) { p1.style.height = Math.floor(p1.style.height.replace('px','') / 2) * 2 + 2 + 'px' } else { p1.style.height = Math.ceil(p1.style.height.replace('px','') / 2) * 2 + 'px' }
  ...
  if (a.style != null && !yt6.ytg) {
    ...
    yt6.pls = (!yt6.mobile) ? gid('watch-appbar-playlist') || gid('playlist') || gid('queue') || document.getElementsByTagName('ytg-playlist-panel-renderer')[0] : document.getElementsByTagName('lazy-list')[0] //|| document.getElementsByTagName('ytd-live-chat-frame')[0] || gid('chat')
...
function control_panel1() {
  ...
  if (!wna) return void 0;
    ...
    yt6.pls = (!yt6.mobile) ? gid('watch-appbar-playlist') || gid('playlist') || document.getElementsByTagName('ytg-playlist-panel-renderer')[0] || document.getElementsByTagName('ytd-live-chat-frame')[0] || gid('queue') || gid('chat') : document.getElementsByTagName('lazy-list')[0]
    if (yt6.layout == 16 && yt6.pls && ((!yt6.pls.lastChild || !yt6.pls.firstElementChild) || yt6.pls.hasAttribute('hidden')) ) yt6.pls = null
...
/* Check the class name of the button and act accordingly */    
	...
	try { yt6.player1.pause() } catch(e){}
	  ...
	  function() { var p = player(), p = yt6.p;
	    ...
	    try { yt6.navigation = true
	    if (yt6.layout == 12 || yt6.mobile || yt6.ytg) {
