...
for (i in z) {
  ...
  if (sig == 2) { var aac2 = aac.replace('http://','https://'); sig == null;
   if (aac2 != null) {
    ...
    html.push(
      '<a href="' + href + '" onclick="' + rp(onclick) + '">256k DASH AAC</a>'
    );break
   }
  }
  if (z[i] == "s") var sig = 1;
...
if (srcto == undefined) {\
		...
		success: function(me) {  $('#audio-type').html( me.pluginType);\
					...
					me.addEventListener('ended', function() { Seek = 3; me.pause() });\
					me.addEventListener('pause', function() { if (Seek == 4) { Seek = null; player1.play() }; if (Seek == 2) { Seek = null }; if (!player1.paused) { player1.pause() } });\
					me.addEventListener('play', function() { Seek = 2 ;var player1_src = document.getElementById('player1').getAttribute('src').replace('&ratebypass=yes',''); if ( (player1_src.slice(-2) == '&2')  ) {Seek = 4; me.pause() }; if ( (!player1.playing) && (me.src != player1_src) ) { player1.play() } else { Seek = 4; me.pause()} } );\
...
var Seek = null;var p2muted = false; var cvol = player2.volume;\
   var watchit = setInterval(function(){ \
if (typeof player2 === 'undefined') { clearInterval(watchit) } else { \
  if (typeof player.getCurrentTime == 'function') {\
var diff = parseFloat(player2.currentTime) - parseFloat(player.getCurrentTime() );\
if ( ((parseFloat(diff)) > parseFloat(1.2)) || ((parseFloat(diff)) < parseFloat(-1.2)) ) { Seek = 1 };\
if ( ((parseFloat(diff)) > parseFloat(0.9)) || ((parseFloat(diff)) < parseFloat(-0.9)) ) { if (Seek === 1) { if (!player2.muted) { p2muted = false; player2.muted = true}; player2.currentTime = parseFloat(parseFloat(player.getCurrentTime()) - parseFloat(0.14));}};\
;\
if ( ((parseFloat(diff)) < parseFloat(0.25)) && ((parseFloat(diff)) > parseFloat(0.00)) )  { Seek = null; if (!p2muted) { p2muted = true; player2.muted = false;} };\
}\
  }\
   },1000);\
 }\
...
//document.getElementById('player2').playbackRate = 2.0;
...
if (document.getElementById("bm2").style.visibility === "visible") { 
  ...
  csspopupheight = csspopupheight0;
  windowheight = parseInt(window.innerHeight || document.documentElement.clientHeight || document.getElementsByTagName('body')[0].clientHeight);
  ...
  document.getElementById('bm4').onclick = onclick;
...
if ((lang_def == null) && (lang_asr == null)) { window.open("https://www.youtube.com/html5", "_blank").focus() }
  ...
  document.getElementById('bm5').onclick = onclick;
}
...
if (dw != null) { dw.remove() }
  dw = document.createElement('div');
  dw.id = 'aspect';
  dw.innerHTML = '<input id="a_width" value="' + w.replace("px","") + '" style="display:inline-block;width:30px;height:13px;text-align:right"></input>x<input id="a_height" value="' + h.replace("px","") + '" style="display:inline-block;width:30px;height:13px;text-align:right"></input>px';
...
//class="player-width player-height off-screen-target player-api"
   ...
   document.getElementById('watch7-sidebar').style.marginTop = '0px';
...
if (typeof ytcenter !== 'undefined') { js.kind = parseInt(parseInt(h.replace('px',''))+parseInt(30))} else {js.kind = h.replace('px','')}
document.getElementById('watch7-sidebar').style.marginTop = '-'+js.kind+'px';
document.getElementById('player-api').style.height = h.replace('px','')+'px';
