		...
		if (yt6.x) {
		  if (typeof t1 == 'number' && typeof t2 == 'number' && Math.abs(t1 - t2) > 0.3) {
		    if (!(yt6.audiox && t1 > yt6.player2.media.duration)) {
		      yt6.ct = yt6.player2.media.currentTime = yt6.player1.media.currentTime = t1
		    }
		  }
...
function ageless_verification(spfpc) { //console.log('age')
  ...
  if (vid) {
	...
	/*if (c[0] && c[0].loaded_from == 'embed') {
	  yt6.ytplayer = clone(ytplayer)
	  yt6.ytplayer.config.loaded_from = 'embed'
	  if (yt6.ytplayer.config.args) {
	    var x = yt6.ytplayer.config.args.player_response
	    if (x && x.indexOf('"') > -1) { console.log((c[0] == ytplayer.config))
		if (x.indexOf('%7B') == 0) {
		  x = x.split('%22').join('\\%22')
		  x = x.split('%2C').join(',')
		  x = x.split('%7B').join('\\{')
		  x = x.split('%7D').join('\\}')
		}
		...
		x = x.split('"').join('\\%22')
		x = x.split('}').join('\\}')
		//.split('\\"').join('%22').split('\\u0026').join('//u0026').split('\\').join('').split('//u0026').join('\\u0026'))
		yt6.ytplayer.config.args.player_response = x
	    }
	  }
	}*/
...
//console.log(document.title + yt6.vid + window.ytplayer.config.args.video_id + video_id()[0]);
	...
	if (yt6.age.t) {
	  yt6.blocked = yt6.age.t
	    ...
	    yt6.blocked = (yt6.blocked) ? yt6.blocked[yt6.txt].replace(/(\r\n|\n|\r)/gm," ").split(' ').join('') : false; //console.log(yt6.blocked +' '+yt6.age.blocked)
	  }
	...
	if (yt6 && (!yt6.x || yt6.browser_tab == 'hidden' || (yt6.x && yt6.navigation && yt6.browser_tab == 'visible') || (yt6.mobile && yt6.x) )) {
	  ...
	  switch (newState) {
		...
		case 1: if (yt6.x && yt6.navigation && yt6.browser_tab == 'visible') try { yt6.p.pauseVideo(); } catch(e){};
			...
			if (!yt6d.ended && p2()) {
			  if ( player2.src == yt6.audiox && yt6.p.firstChild && yt6.p.firstChild.firstChild && yt6.p.firstChild.firstChild.src && yt6.p.firstChild.firstChild.src.indexOf('http') == 0) { player2.play(); break }//typeof srcto != 'undefined' && srcto
			  if (v && v.src) {
		...
		case 2: if (!yt6.x && p2() && yt6.player2 && yt6.player2.media && !yt6.player2.media.paused) { player2.pause(); resync() }
...
function getReferenceObjects() {
  ...
  yt6.osw = (!yt6.ytm) ? gid('placeholder-player') || gid('player') : document.getElementsByTagName('ytmusic-player')[0]
  yt6.mhp = (!yt6.ytm) ? gid('masthead-positioner') || gid('logo') || gid('masthead-logo') : document.getElementsByTagName('ytmusic-nav-bar')[0];//gc('x-scope ytd-masthead-1')[0]
  if (!yt6.mhp && !yt6.ytm) {
    var z = gid('home-button')
    if (z) {
	...
	if (yt6.prefer_fmt == 'all') {
	  ...
	  for(j=index;j<fmt_pool.length;j++){
	    ...
	    //} else {
	      ...
	      if (autoplay() && !yt6.ytm && (yt6.x || (!yt6.x && yt6.p && yt6.p.firstChild && yt6.p.firstChild.firstChild && yt6.p.firstChild.firstChild.src && yt6.p.firstChild.firstChild.src.indexOf('http') == 0) || yt6.browser_tab == 'hidden' || !yt6d.previous.linx.length) ) {
...
//			  }
    ...
    } else if (yt6.layout == 16 && yt6.status == 'unplayable' && yt6.p.tagName == 'IFRAME') {
	yt6.vid = video_id()[0]
	//iframe()
      }*/
...
*/
  ...
  for (i in ft) {
	...
	var linx_bak = href
	  ...
	  var l
	  // look for differing length, may be advertisement
...
'			};');
	    ...
	    var z = 1 * ( y - 1 * qq.length - 0 - 3*Math.floor((1 * qq.length/18) - 0) ); //console.log('Array '+ z + typeof z)
            if (typeof z == 'number' && z > 0 && qq.indexOf('+') == -1) {
              ...
              sizeq = ' title="'+ m +' ('+itag_+')" aria-label="'+ m +' ('+itag_+')"'
	      if (m == 'V') yt6.yt_otf.push(1*itag_)
            } 
...
if (typeof html.splice != 'function') return void 0;
  ...
  html.push(
   ...
   '<div id="yt6-switches">'+
   '<div style="inline-block">Preferred format in case <a href="#" onclick="alert(&quot;' + navigator.userAgent + '&quot); return false" style="color: red" title="Click to see current User-Agent string" aria-label="Click to see current User-Agent string">' + browserName + '</a> is having trouble playing one back:<br>'+
...
'<button id="wp_t" class="snarl-button yt-uix-button-text" onclick="var yt6 = gid(\'snarls_player\'), wp_cb = gid(\'wp_cb\'); if (yt6.tbg && yt6.tbg.style.left) this.wp_left = clone(yt6.tbg.style.left); if (yt6.wallpaper) { if (typeof yt6.wallpaper == \'string\') { yt6d.wallpaper = clone(yt6.wallpaper); yt6.wallpaper = prompt(\'Enter URL(s) of custom wallpaper(s) (separated by comma)\\nType &quot;1&quot; to restore default.\\n\\nImages can be of any type which the browser can display. They will be hotlinked this way, so avoid linking to sites with strict download limits and prefer those served through caching servers.\', yt6d.wallpaper); if (yt6.wallpaper == 1) { yt6.wallpaper = false; yt6d.wallpaper = \'' + protocol() + yt6.cdn + '31ec5334c730af96803fb1b3cc677eb0c5e5c0df/extrawide.jpg\' }; if (yt6.wallpaper == undefined) { yt6.wallpaper = true; this[yt6.txt] = \'Thumbnails wallpaper\'; } else { if (!yt6.wallpaper && wp_cb && wp_cb.hasAttribute(\'checked\')) { wp_cb.removeAttribute(\'checked\'); wp_cb.checked = \'\'; yt6.wallpaper = false; this[yt6.txt] = \'Thumbnails wallpaper\'; } else this[yt6.txt] = \'Custom wallpaper\'; }; if (typeof yt6.wallpaper == \'string\' && (yt6.wallpaper.indexOf(\'https://\') == 0 || yt6.wallpaper.indexOf(\'http://\') == 0)) yt6d.wallpaper = clone(yt6.wallpaper); } else { this[yt6.txt] = \'Custom wallpaper\'; yt6.wallpaper = clone(yt6d.wallpaper) } } else { if (wp_cb) { wp_cb.setAttribute(\'checked\',\'checked\'); wp_cb.checked = \'true\'; yt6.wallpaper = true } }; wallpaper(); if (this.wp_left && yt6.tbg) yt6.tbg.style.left = clone(this.wp_left); ">' + ((typeof yt6.wallpaper == 'string') ? 'Custom wallpaper' : 'Thumbnails wallpaper') + '</button>'+
  ...
  var dw = document.createElement('button')
  dw.id = "bm5";
  dw.innerHTML = 'User-Agent compatibility check'
  ...
  gid('bm5').setAttribute('onclick', 'window.open( "' + protocol() + '//www.youtube.com/supported_browsers", "_blank").focus() ')
...
//i1.ytimg.com/i/gsZadWjuuN2dEhI0mZfVfQ/mq1.jpg?v=531525b4
  ...
  var onclic = function(){
    ...
    var mhp = (yt6.layout != 16 || yt6.ytm) ? yt6.mhp.offsetHeight : yt6.mhp.parentNode.parentNode.offsetHeight
    if (mhp < 30 || mhp > 112) mhp = 60
    var bm3 = gid('bm3')
    if (bm3) bm3.style.top = mhp - 2 + 'px'//(!yt6.ytm) ?  : '0px'
					  ...
					  if (sauce) {
					    if (a && b) {
						  ...
						  if (yt6.ads_ei) if (z[j].href && z[j].href.indexOf(yt6.ads_ei) > -1) z[j].href = z[j].href.split('&title=')[0] + '&title=Advertisement'
						    ...
						    a.appendChild(z[j]); a.appendChild(z[j-1])
					            ...
					            if (y) for(k=0;k<y.length;k++){
					              if (y[k].name && y[k].href && y[k].name == itag(sauce[i].value) && y[k].href.indexOf(sauce[i].value) == -1) { b.appendChild(y[k]);b.appendChild(y[k-1]); b.insertBefore(b.lastChild, b.lastChild.previousSibling); break }
					            }
					            break
					          }*/
					        }
					  ...
					  if (me.networkState == 1 || me.networkState == 3) {
						...
						function uniq(a) {
						    ...
						    return a.filter(function(item) {
								...
								// || (!(yt6.userprefV[0] || item && yt6.userprefV[1] == item) && yt6.yt_otf.indexOf(item) > -1)
								) ? false : (seen[item] = true);
						    });
						}
						...
						if (!(yt6.navigation && !yt6.linx[itag_]) || (yt6.navigation && (!yt6.linx.length || (yt6.linx.length && 3 >= yt6.linx.length))) ) {
						  ...
						  if (autoplay(false) && (yt6.x ) ) {//|| (yt6.layout == 16 && yt6.browser_tab == 'hidden') 
...
/*					      var dash = gid(mep_x('mep_') + '_sourcechooser_242')//240p  VP9webm');
					...
					addEL(me, 'loadeddata', function() { //console.log('1loaded '+ me.loaded_vid); 
					  if ( yt6 && yt6.player1 && !yt6.navigation && (yt6.timer == 999999999 || me.src == 'https://www.youtube.com/ptracking' || yt6.player1.media.src != me.src))
					    return void 0;
					...
					addEL(me, 'play', function() { //console.log('1play')
					  if ( yt6 && yt6.player1 && !yt6.navigation && (yt6.timer == 999999999 || me.src == 'https://www.youtube.com/ptracking' || yt6.player1.media.src != me.src || (yt6.browser_tab == "hidden" && yt6.diff > 0.6 && me.currentTime > 0 && !(yt6.V_[itag(me.src)] && yt6.player2.media.paused)) )) { //console.log(yt6.diff + ' == '+me.currentTime)//m2
					    return void 0 }
					  if (yt6.x) {
					  ...
					  if ( ((!yt6.A_V[itag(me.src)] &&
						 ...
						 || (Seek != 4 && player2.src == yt6.audiox) //(typeof srcto != 'undefined' && Srcto == Audio)
						  ...
						  } else if (yt6.player2.media.src != yt6.audiox && ((yt6.x && yt6.browser_tab == 'hidden') || !yt6.player2.media.loaded)) { //m2
						      yt6.player2.load()
						      if (yt6.player2.media.currentTime > me.currentTime) me.setCurrentTime(yt6.player2.media.currentTime)
						  ...
						  if (!yt6.A_V[itag(me.src)] && player2.duration != player2.currentTime && !(me.currentTime > player2.duration)) {// && yt6.browser_tab == 'visible'
							...
							if (yt6.browser_tab == "hidden") {//m2
							  if (me.loaded_vid == yt6.vid && !yt6.navigation && yt6.title == document.title && me.currentTime && Math.abs(me.duration - me.currentTime) > 1 ) {
							    yt6.Seeked2 = false; Seek = 1;
							    yt6.ct = me.currentTime = yt6.player1.media.currentTime = clone(yt6.player2.media.currentTime)
							  }
							} else {
					    ...
					    if (!me.paused && typeof yt6.player2.media.loaded != 'number') { //m2
					...
					addEL(me, 'pause', function() { //console.log('1pause ' + Seek + ' ' + me.currentTime +' '+ yt6.player2.media.currentTime )
					  ...
					  if (yt6.browser_tab == 'hidden' && ((Seek != 3 && me.currentTime < me.duration) || (Seek == 3 && player1.options.loop1)) ) {
					    if (yt6.x) {
					      yt6.Seeked2 = true
					      if ((yt6.x && yt6.browser_tab == 'hidden') || 0.3 >= yt6.diff || player2.currentTime == 0) {// 0.3 >= yt6.diff condition is needed to start from the lock screen
						me.play(); //console.log('player1 resuming playback on hidden tab')
						if (yt6.md && yt6.browser_tab == 'hidden') if (yt6.V_[itag(me.src)]) { yt6.Seeked2 = false } else $waitUntil(function(){ if (!yt6.navigation && yt6.browser_tab == 'hidden' && yt6.player1.media.loaded_vid == yt6.vid && yt6.player1.media.paused) return true },function(){ if (!yt6.swapped && yt6.Seek != 6 && Seek != 3) yt6.player1.media.play() },500,5000)
					      }
					    }; return void 0
					  }
					  if (yt6.x) {
					...
					addEL(me, 'timeupdate', function() {
					  ...
					  if (yt6.browser_tab == 'visible' && yt6.Seek == 6) {
					    ...
					    } else { yt6.ct = me.currentTime = yt6.player1.media.currentTime }
					    Seek = null
					  } else
					  if (yt6.x) {
					  ...
					  if (player1.options.loop1) {
					    ...
					    try { yt6.player1.pause(); yt6.player2.pause(); Seek = 1;
					     ...
					     if (yt6.A_V[itag(me.src)] || yt6.A_[itag(me.src)]) { me.play() }//for looping singular streams in player1 only
						  ...
						  if (yt6.layout == 12 && yt6.x && yt6.browser_tab == 'hidden') { // mobile sleep mode hack
						    try {
						  ...
						  if (z && z[0]) {
							...
							//try { yt6.player1.media.currentTime = player2.currentTime = 0 } catch(e) { yt6.player1.media.currentTime = 0 };
							  if (z[i+1]) {
							    ...
							    if (yt6.browser_tab == 'hidden' && yt6.layout == 16 && yt6.x && yt6.pls && !yt6.mobile && !yt6.ytg) {//!yt6.ytm && 
					...
					addEL(me, 'playing', function() { //console.log('1playing '+ Seek +' '+ player1.media.playing +' '+ yt6.player2.media.playing)
					  ...
					  if (yt6.V_[itag(me.src)]) {//m2
					    if (player2.paused) { Seek = 1; yt6.player2.play() }
					  }
					  if (Seek == 1 && yt6.browser_tab == 'visible' && (!yt6.mobile || (typeof me.currentTime == 'number' && me.currentTime > 1)) ) {
					    if (Math.abs(yt6.player1.getCurrentTime() - yt6.player2.getCurrentTime()) < 0.1) { Seek = 2 }
					  }
	...
	if (typeof srcto == 'undefined') {
		yt6.player2 = new MediaElementPlayer('#player2',{
		  ...
		  success: function(me) {  $('#audio-type').html( me.pluginType);
					addEL(me, 'error', function(e) { //console.log('2error: ' + me.networkState + ', media type: "'+ me.canPlayType(me.type) + '"')
					  ...
					  if ((me.networkState == 1 || me.networkState == 3) && yt6.retry < yt6.limit) {
						...
						yt6.player1.pause()
						$waitUntil(function() { if (yt6.player1.media.loaded && yt6.player1.media.paused) return true }, //m2
						function() { yt6.player1.play(); yt6.player1.play(); me.play() },250,2500);
					    } else {
						Seek = 0;
					...
					addEL(me, 'loadeddata', function() { //console.log('2loaded')
					  if ( yt6 && yt6.player2 && !yt6.navigation && (yt6.timer == 999999999 || me.src == 'https://www.youtube.com/ptracking' || yt6.player2.media.src != me.src || !gid('player1') ))
					    return void 0;
					  me.loaded_vid = video_id()[0]; me.loaded_itag = itag(me.src)
					  if ( yt6.V_[player1.loaded_itag] && (
						(yt6.browser_tab == 'hidden' && ((yt6.md && yt6.mobile) || (!yt6.md && yt6.layout == 16)) ) || //!(yt6.layout == 16 && !yt6.mobile && !yt6.ytm && !yt6.ytg && yt6.browser_tab == 'hidden') ||
					...
					addEL(me, 'seeked', function() { //console.log('2seeked '+ Seek +' '+ yt6.player2.media.currentTime)
					...
					addEL(me, 'pause', function() { //console.log('2pause '+ Seek +' '+ yt6.player2.media.currentTime)
					  if ( yt6 && yt6.player2 && !yt6.navigation && (yt6.timer == 999999999 || me.src == 'https://www.youtube.com/ptracking' || yt6.player2.media.src != me.src))
					    return void 0;
					  if (yt6.browser_tab == 'hidden' && ((Seek != 3 && me.currentTime < (me.duration-1)) || (Seek == 3 && player1.options.loop1) || (!yt6.x && yt6.p.tagName == 'DIV' && yt6.ytp && yt6.ytp.v && typeof yt6.ytp.v.currentTime == 'number' ) ) ) { //m2 && yt6.V_[itag(yt6.player1.media.src)]
					     ...
					     me.play()
					     if (yt6.x && yt6.md && yt6.browser_tab == 'hidden') $waitUntil(function(){ if (!yt6.navigation && yt6.browser_tab == 'hidden' && yt6.player1.media.loaded_vid == yt6.vid && yt6.player1.media.paused) return true },function(){ if (!yt6.swapped && yt6.Seek != 6 && Seek != 3) yt6.player1.media.play() },500,5000)
					     return void 0
					  }
					  if (yt6.x || (player() && typeof yt6.p.getPlayerState != 'function')) {
					    ...
					    if (yt6.x) if (Seek == 4) { //console.log('AV format selected, separate audio paused')
					      player1.play()
					    } //else if (!yt6.player1.media.paused && !(yt6.md && yt6.browser_tab == 'hidden')) { player1.pause() }
						...
						if (me.src != player1_src) {
						  if (!yt6.player1.media.paused || yt6d.init) { //console.log(me.loaded); console.log(yt6.player2.media.loaded); console.log(yt6.player1.media.loaded)
						    ...
						    if (typeof me.loaded == 'number') {
						      ...
						      if (!yt6.player1.media.paused) { Seek = 2 } else { Seek = 1; yt6.Seeked2 = (!yt6d.init) ? true : false }
...
yt6.A_V = AV
  ...
  if (ytplayer && ytplayer.config && ytplayer.config.args) {
    ...
    if (ytplayer.config.args.ttsurl && unescape(ytplayer.config.args.ttsurl) != 'undefined') {
...
//
	...
	if (!yt6.x && !yt6.ytm && ivo.indexOf(yt6.vid + '$done' + yt6.ads_on) == -1 && p && p.tagName == 'DIV') {
	  ...
	  if (ivo != yt6.vid + '$done') {
	    var ads = gc('ytp-ad-overlay-slot')[0]; if (ads) { ads.style.display = 'none'; ivo = yt6.vid + '$done' }
	  }; console.log('"'+ivo+'"')*/
	} else {
		    if (yt6.ytm && bm0) {
...
//console.log('? '+c[1].video_id +' '+yt6.vid)
	  ...
	  if (c[1] && c[1].video_id && (c[1].video_id != yt6.vid || yt6.blocked) ) { //typeof c[1].video_id == 'string' && (c[1].video_id != yt6.vid) //|| (yt6.ytm &&
	    ...
	    if ( (yt6.blocked
		   ...
		   && document.title != yt6.title
		   && (!(yt6.ytm && yt6.ad_ && yt6.loaded_vid == yt6.vid) || yt6.blocked) ) {// reloading may be triggered twice if these values are still the same at this point, so wait another turn till yt updates document.title
...
if ((p1 != null) && (yt6.x)){
  ...
  for(i=0;i<x.length;i++){
	...
	if (Math.abs(h - 30) < src_chooser_height) {
...
function switch_players(iframe) {
  ...
  if ( !!bm0 && bm0.style.visibility === 'hidden') { // && !!p && p.style.visibility != 'hidden'
	...
	yt6.player2.media.currentTime = p.getCurrentTime(); p.playVideo(); p.pauseVideo(); yt6.player2.media.pause()
      ...
      try { if (p.isMuted()) { yt6.ytp.vol = 0 } else yt6.player2.media.volume = parseFloat(p.getVolume() / 100) } catch(e){}
...
function control_panel1() {
  ...
  if (gid('remove-sp') == null) {
    ...
    remove.id = 'remove-sp'
    var remove_sp_innerhtml = '<button onclick="switch_players()" class="snarl-button yt-uix-button-text" aria-label="Switch" title="Switch" style="width: 22px; height: 22px; padding: 0 1px 0 0"><img src="//s.ytimg.com/yts/img/HTML5_1Color_Black-vfl902gVJ.png" style="vertical-align:middle; height:12px; padding:0px""></img></button><button onclick="aspect()" class="snarl-button yt-uix-button-text" style="width: 30px" aria-label="Resize" title="Resize">«↔»</button><br><button style="width: 22px" onclick="var yt6 = gid(\'snarls_player\'); var z = window.prompt(\'Wanna quit? \(Exit options: 0-3\)\\n Cancel = Not now;\\n 0 = Remove script without page refresh;\\n 1 = Classic layout;\\n 2 = Material Design layout;\\n 3 = Visit page on YT Music;\', \'0\'); if (!z) var z = window.confirm(\'OK will attempt to switch between YouTube layouts and refresh the page. \\nCancel will abort.\'); if (typeof z == \'boolean\' && z == false) return void 0; if (z && !(typeof z == \'boolean\' && z == true)) { if (z != 0 && (z == 1 || z == 2 || z == 3)) { if (z == 2) document.cookie=&quot;PREF=f1=50000000&f5=10&f6=1004;path=/;domain=.youtube.com;expires=Session&quot;; if (z == 1) { document.cookie=&quot;PREF=f1=50000000&f5=10&f6=8;path=/;domain=.youtube.com;expires=Session&quot;; if (yt6.layout != 12) alert(&quot;Spoof the User-Agent to get the old layout. Replace the included OS name with the case-sensitive string \'Googlebot\'.&quot;); }; if (z == 3) { var domain = \'music\'; location.href = window.location.href.replace(\'www.youtube.com\', domain + \'.youtube.com\').replace(\'music.youtube.com\', domain + \'.youtube.com\').replace(\'gaming.youtube.com\', domain + \'.youtube.com\').replace(\'m.youtube.com\', domain + \'.youtube.com\') } else { location.href = window.location.href.replace(\'gaming.youtube.com\',\'www.youtube.com\').replace(\'music.youtube.com\',\'www.youtube.com\').replace(\'m.youtube.com\',\'www.youtube.com\'); if (!yt6.ytm) location.reload() } } else if (z == 0) deldiv() } else { if (!yt6.ytg) if (yt6.layout == 16) { document.cookie=&quot;PREF=f1=50000000&f5=10&f6=8;path=/;domain=.youtube.com;expires=Session&quot;; } else document.cookie=&quot;PREF=f1=50000000&f5=10&f6=1004;path=/;domain=.youtube.com;expires=Session&quot;; location.href = window.location.href.replace(\'gaming.youtube.com\',\'www.youtube.com\').replace(\'music.youtube.com\',\'www.youtube.com\'); };" class="snarl-button yt-uix-button-text" aria-label="Remove script / Change YouTube layout" title="Remove script / Change YouTube layout">X</button><button id="audio_x" class="snarl-button yt-uix-button-text" aria-label="External audio" title="External audio" style="padding: 0 1px 0 1px; right: 0px" onclick="var yt6 = gid(\'snarls_player\'); if (yt6.player1 && yt6.player2 && typeof yt6.player2.load == \'function\') { var audio_x = window.prompt(\'Enter URL of any audio (or video) file across the net to use as background sound. Only plays alongside non-MSE-mode video-only sources.\', gid(\'no2\').src); yt6.audiox = gid(\'no2\').src = yt6.player2.media.src = audio_x || yt6.linx[yt6.userprefA[0]]; if (!audio_x) { yt6.audiox = null; delete yt6.audiox }; yt6.player1.pause(); yt6.player2.load() };">audio</button></div>'
