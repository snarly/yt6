...
function correct_flashvars(a) { // prepare the flash player object's flashvars variable
  ...
  if (c[0] && !(c[1] && c[1].video_id)) {					// brand new since 2019-07-30: the video_id object is deleted by yt on each new page
    a.video_id = vid
			...
			function(){
			  if (gid('movie_player') && gid('movie_player') != yt6.movie_player && gid('movie_player').tagName == 'EMBED' && !gid('movie_player').getAttribute('flashvars')) {
			    ...
			    if (!(z && z.parentNode.parentNode == yt6.movie_player)) { // no sign of error
			      ...
			      yt6.force_flash = false; console.log('1')
			...
			if (iframe) {//if (z.re2 != true) {
			  ...
			  if (yt6.movie_player && yt6.movie_player.tagName == 'EMBED' && yt6.movie_player.getAttribute('src') == '' && yt6.movie_player.getAttribute('flashvars') == '' && yt6.movie_player.parentNode != y) { // here the initial dummy flashplayer object will be swapped for an iframe -- this may put ytplayer in a dysfunctional state on subsequent videos (at least in Firefox's Material Design layout handling)
			    ...
			    if ((ytplayer.config.args.errorcode == 150 || ytplayer.config.args.errorcode == 101 || !ytplayer.config.args.errorcode)) {// remove it if embedding is desabled
			      ...
			      yt6.force_flash = false; yt6.flash = null; console.log('2')
			    }
			  }
...
if (c[1]) {
  ...
  var ft = [args.url_encoded_fmt_stream_map, args.adaptive_fmts];// console.log(ft)
...
/*					      var dash = gid(mep_x('mep_') + '_sourcechooser_242')//240p  VP9webm');
						...
						if ( (yt6.ytg && yt6.errcount >= 1) || (!yt6.ytg && yt6.errcount >= (1 * proxies.length)) ) {//(yt6.ytg && !(yt6.errcount < (1 + 1 * proxies.length)) //me.src = '';
						  ...
						  // let's try flash playback
							...
							var z = gid('flash_on')
							if (z) { z.checked = false; z.setAttribute('checked','') }
							yt6.force_flash = false; console.log('3')
...
if (!yt6.ytm && !yt6.delay_flash_actions && (yt6.force_flash || (p.tagName == 'EMBED' && p.getAttribute('class') == 'forced flashplayer')) ) { //console.log(yt6.ct +' '+ yt6.loaded_media_duration + ' '+yt6.real_media_duration_)
  if (yt6.autoplay && !yt6.navigation && ((yt6.x && yt6.loaded_media_duration && yt6.player1.media.paused && yt6.ct >= yt6.loaded_media_duration - 1) || (!yt6.x && p && typeof p.getPlayerState == 'function' && p.getPlayerState() == 0 && p.getCurrentTime() >= p.getDuration() )) ) {
					          var z = gclass('yt-uix-scroller-scroll-unit')
						  if (z && z[0]) {
		...
		if (yt6.blocked && video_id()[0] != prev_vid){//(yt6.layout != 16 || yt6.blocked_vid0)) {//yt6.age.check() && video_id()[0] != prev_vid
		  ...
		  //    if (yt6.age.done == 2) {
			...
			if (c[1] && c[1].video_id.substring(11) || (yt6.ytm && document.title == 'YouTube Music')) {// != 'ytm'
			  ...
			  } else if (yt6.ytm && document.title == 'YouTube Music') {console.log('??!!')
	...
	case 'reset snarl-button yt-uix-button-text':
	  ...
	  if (v != p) {
	    ...
	    if (webgl) {
	      ...
	      var opacity = (player()) ? player().style.opacity : '0.9';
	      if (yt6.layout == 16) {
	        p.style.display = 'inline-block'; if (yt6.movie_player) var display = yt6.movie_player.style.display;
